...
void KittenBot::init(){
  ...
  Timer1.initialize(100); //100us, 255*100 = 25ms
...
void KittenBot::stepArc(float R, float degree){
  ...
  W = V/R;
  float spdRatio;
  float theta = -degree/180*PI/2; // change to rad
...
void KittenBot::omniWheel(int vspeed, int hspeed, int rspeed){
...
void KittenBot::servoDegree(int pin, int degree){
	servo[10].attach(pin);
	servo[10].write(degree);
}

void KittenBot::servoArray(int idx, int degree){
