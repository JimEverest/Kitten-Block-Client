...
void Adafruit_LEDBackpack::setBrightness(uint8_t b) {
  if (b > 15) b = 15;
  swWire.beginTransmission(i2c_addr);
  swWire.write(HT16K33_CMD_BRIGHTNESS | b);
...
void Adafruit_LEDBackpack::blinkRate(uint8_t b) {
  ...
  swWire.write(HT16K33_BLINK_CMD | HT16K33_BLINK_DISPLAYON | (b << 1)); 
  swWire.endTransmission();
}
...
void Adafruit_LEDBackpack::begin(uint8_t _addr = 0x70) {
  ...
  swWire.beginTransmission(i2c_addr);
  swWire.write(0x21);  // turn on oscillator
  swWire.endTransmission();
...
void Adafruit_LEDBackpack::writeDisplay(void) {
  swWire.beginTransmission(i2c_addr);
  swWire.write((uint8_t)0x00); // start at address $00

  for (uint8_t i=0; i<8; i++) {
    swWire.write(displaybuffer[i] & 0xFF);    
    swWire.write(displaybuffer[i] >> 8);    
  }
  swWire.endTransmission();  
}
...
/****** kittenbot nekomimi 16x8 matrix *********/

Kittenbot_16x8matrix::Kittenbot_16x8matrix(void) : Adafruit_GFX(16, 8) {
}

void Kittenbot_16x8matrix::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_7segment::writeColon(void) {
    swWire.beginTransmission(i2c_addr);
    swWire.write((uint8_t)0x04); // start at address $02
    
    swWire.write(displaybuffer[2] & 0xFF);
    swWire.write(displaybuffer[2] >> 8);
