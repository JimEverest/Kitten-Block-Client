 ...
 ******************************************************************************/
bool RFID::isCard(unsigned char *str)
{
...
void RFID::init()
{
	#if(RFID_TRANSFER_MODE == RFID_SPI)
		SPI.begin();
	#elif(RFID_TRANSFER_MODE == RFID_I2C)
		Wire.begin();
	#elif(RFID_TRANSFER_MODE == RFID_UART)
		Serial.begin(9600);
	#endif
  ...
  //Serial.println(readMFRC522(TModeReg),HEX);
  
  writeMFRC522(TPrescalerReg, 0x3E);  //TModeReg[3..0] + TPrescalerReg
  //Serial.println(readMFRC522(TPrescalerReg),HEX);
  
  writeMFRC522(TReloadRegL, 30);
  //Serial.println(readMFRC522(TReloadRegL),HEX);
...
void RFID::writeMFRC522(unsigned char addr, unsigned char val)
{
		...
		Wire.beginTransmission(MFR_DEVICE_ADDRESS);
		Wire.write(addr);//选择地址
		Wire.write(val);
...
unsigned char RFID::readMFRC522(unsigned char addr)
{
  ...
  unsigned int i=5000;
		...
		Wire.beginTransmission(MFR_DEVICE_ADDRESS);
		Wire.write(addr);//选择寄存器地址
		Wire.endTransmission();

		Wire.requestFrom(MFR_DEVICE_ADDRESS, 1);
		do
		{
			i--;
		}
		while (!Wire.available()&&i!=0);
		val = Wire.read();
	#elif(RFID_TRANSFER_MODE == RFID_UART)
		Serial.write(addr | 0x80);
		while(!Serial.available()){}
		val = Serial.read();
	#endif
