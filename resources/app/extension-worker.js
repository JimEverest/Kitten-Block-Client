...
var ExtensionWorker = function () {
    ...
    _createClass(ExtensionWorker, [{
        ...
        value: function register(extensionObject) {
            ...
            var serviceName = 'extension.' + extensionObject.getInfo().name;
    ...
    _createClass(SharedDispatch, [{
        ...
        value: function _onMessage(worker, event) {
            ...
            if (message.service) {
                ...
                if (message.result && message.result.devmsg) {
                    var msg = message.result;
                    msg.responseId = message.responseId;
                    if (window.conn) {
                        window.conn.send(JSON.stringify(msg));
                    }
                    if (!msg.parser) {
                        this._deliverResponse(message.responseId, 0);
                    } else if (typeof msg.retry === 'undefined') {
                        this.timoutHandler = setTimeout(function () {
                            window.conn.send(JSON.stringify(msg));
                        }, 1000);
                    } else if (typeof msg.timeout !== 'undefined') {
                        this.timoutHandler = setTimeout(function () {
                            _this2._deliverResponse(message.responseId, 0);
                        }, msg.timeout * 1000);
                    }
                } else {
